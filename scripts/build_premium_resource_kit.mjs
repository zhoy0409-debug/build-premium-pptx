...
// 05 — agenda
{
  ...
  items.forEach(([n, h, b], i) => {
    const x = 72 + i * 284;
    if (i) rect(s, x - 22, 316, 1, 246, C.line);
    text(s, n, x, 314, 92, 72, { size: 50, bold: true, color: i === 1 ? C.gold : C.green, typeface: SERIF });
    text(s, h, x, 402, 220, 36, { size: 24, bold: true });
    text(s, b, x, 456, 216, 70, { size: 16, color: C.gray });
  });
}
...
// 06 — statement
{
  ...
  text(s, "真正高级的演示，\n不是信息更多，而是判断更清楚。", 118, 140, 1044, 168, { size: 50, bold: true, align: "center", lineSpacing: 0.92 });
  smallRule(s, 520, 350, 240, C.gold);
  const points = [["少", "只保留推动结论的信息"], ["准", "标题直接说出判断"], ["稳", "整套演示保持一个视觉系统"]];
  points.forEach(([h, b], i) => {
    const x = 198 + i * 302;
    text(s, h, x, 410, 160, 46, { size: 28, bold: true, color: C.green, align: "center" });
    text(s, b, x - 44, 472, 248, 54, { size: 16, color: C.gray, align: "center" });
  });
}
...
// 07 — image + text frame
{
  ...
  rect(s, 72, 296, 632, 316, C.pale, { radius: "rounded-2xl", name: "replaceable-visual-frame" });
  rect(s, 104, 328, 568, 252, C.sage, { radius: "rounded-xl" });
  text(s, "可替换主视觉", 218, 414, 340, 48, { size: 26, bold: true, color: C.forest, align: "center", vAlign: "middle" });
  addBullet(s, 1, "先给结论", "标题直接告诉听众这张图意味着什么。", 752, 316, 414);
  addBullet(s, 2, "再给证据", "只保留能支持判断的数字、事实和来源。", 752, 414, 414);
  addBullet(s, 3, "最后给行动", "明确下一步、风险或需要作出的决定。", 752, 512, 414);
}
...
// 08 — visual gallery
{
  ...
  captions.forEach(([n, h, b], i) => {
    const x = 72 + i * 382;
    rect(s, x, 300, 342, 210, i === 1 ? C.sage : C.pale, { radius: "rounded-xl", name: `gallery-frame-${i + 1}` });
    text(s, n, x, 530, 48, 24, { size: 14, bold: true, color: C.gold, typeface: SERIF });
    text(s, h, x + 54, 526, 100, 28, { size: 19, bold: true });
    text(s, b, x, 570, 330, 36, { size: 15, color: C.gray });
  });
}
...
// 09 — evidence columns
{
  ...
  const cols = [["72%", "需求已经出现", "受访者主动提出更快、更简单的流程。"], ["2.4×", "效率可以提升", "重复性工作被压缩，团队把时间留给判断。"], ["18%", "风险仍需管理", "必须保留人工复核和来源追踪。"]];
  cols.forEach(([metric, h, b], i) => {
    const x = 72 + i * 382;
    if (i) rect(s, x - 22, 306, 1, 270, C.line);
    text(s, metric, x, 316, 310, 78, { size: 50, bold: true, color: i === 1 ? C.gold : C.green, typeface: SERIF });
    text(s, h, x, 422, 310, 34, { size: 22, bold: true });
    text(s, b, x, 476, 310, 70, { size: 16, color: C.gray });
  });
}
...
// 14 — line chart
{
  ...
  s.charts.add("line", {
    ...
    series: [{ name: "完成度", values: [58, 66, 74, 85, 92], line: { style: "solid", fill: C.green, width: 4 }, marker: { symbol: "circle", size: 8 } }],
    hasLegend: false,
    yAxis: { min: 40, max: 100, numberFormatCode: "0%", majorGridlines: { style: "solid", fill: C.line, width: 1 }, textStyle: { fill: C.gray, fontSize: 12 } },
...
// 19 — quote / proof point
{
  ...
  text(s, "“", 72, 126, 110, 96, { size: 88, bold: true, color: C.gold, typeface: SERIF });
  text(s, "用户不需要学会设计，\n只需要把真实材料交给系统。", 150, 166, 960, 128, { size: 44, bold: true, color: C.white, lineSpacing: 0.94 });
  smallRule(s, 152, 340, 220, C.sage);
  text(s, "— 资源型 skill 的产品原则", 152, 372, 560, 32, { size: 18, color: C.sage });
  const stats = [["1", "推荐方向"], ["2", "必要问题上限"], ["100%", "逐页渲染质检"]];
  stats.forEach(([n, h], i) => {
    const x = 152 + i * 300;
    text(s, n, x, 486, 220, 58, { size: 40, bold: true, color: i === 2 ? C.gold : C.white, typeface: SERIF });
    text(s, h, x, 556, 220, 28, { size: 15, color: C.sage });
  });
}
